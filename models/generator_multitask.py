...
def choose_generator(g_name, image_batch, image_normal_batch):
    if '32' in g_name:
        return Generator_vgg_32({'data': image_batch}), Generator_vgg_32({'data': image_normal_batch}, reuse=True)
    elif '16' in g_name:
        return Generator_vgg_16({'data': image_batch}), Generator_vgg_16({'data': image_normal_batch}, reuse=True)
    elif '8' in g_name:
    ...
    def setup(self, is_training, num_classes):
        ...
        (self.feed(name + 'score_fr')
         ...
         .deconv([4, 4], pool_shape, [2, 2], num_classes, reuse=self.reuse, biased=False, relu=False,
                 name=name + 'deconv/upscore2'))
        (self.feed(name + 'deconv/upscore2', name + 'score_pool4')
    ...
    def setup(self, is_training, num_classes):
        ...
        (self.feed(name + 'deconv/upscore2', name + 'score_pool4')
         ...
         .deconv([4, 4], pool3_shape, [2, 2], num_classes, reuse=self.reuse, biased=False, relu=False,
                 name=name + 'deconv/upscore_pool4'))

        origin_shape = tf.multiply(tf.shape(self.layers['data']), tf.convert_to_tensor([1, 1, 1, 7]))
        (self.feed(name + 'deconv/upscore_pool4', name + 'score_pool3')
